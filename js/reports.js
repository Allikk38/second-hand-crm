 ...
 * @version 1.0.3
 * @changes
 * - v1.0.3: Исправлена логика loadProducts: при найденном кэше прекращает выполнение.
...
export async function loadProducts(forceRefresh = false) {
    ...
    // При отсутствии forceRefresh сначала пробуем кэш
    if (!forceRefresh) {
        ...
        if (cached) {
            ...
            applyFilters();
            notifyProductsChanged();
            return true; // ← кэш найден, прекращаем выполнение
        }
    }
