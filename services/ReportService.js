 ...
 * - Экспорт в различных форматах (подготовка)
 * 
...
const CACHE_TTL = 60000; // 1 минута для отчетов
...
export const ReportService = {
    ...
    /**
     * Внутренний метод получения статистики
     * @private
     * @returns {Promise<Object>}
     */
    async _fetchTotalStats() {
        ...
        products?.forEach(p => {
            ...
            if (p.status === 'in_stock') {
                ...
                totalPotentialProfit += (p.price || 0) - (p.cost_price || 0);
    ...
    async getSalesReport(startDate, endDate) {
        ...
        // Группировка по дням
        const dailyStats = {};
        const hourlyStats = Array(24).fill(0).map(() => ({ count: 0, revenue: 0 }));
        ...
        return {
            ...
            hourly: hourlyStats,
            topProducts,
            sales: sales.slice(0, 100) // Ограничиваем для производительности
        };
    },
    ...
    async getProductsReport() {
        ...
        // Низкооборачиваемые товары (долго в наличии)
        const now = new Date();
    ...
    async getSellersReport(options = {}) {
        ...
        // Получаем все смены с информацией о пользователях
        let query = db
            .from('shifts')
            .select(`
                id,
        ...
        const sellers = Array.from(sellersMap.values()).map(seller => ({
            ...seller,
            averageCheck: seller.totalSales > 0 
                ? seller.totalRevenue / seller.totalSales 
                : 0,
            averageShiftRevenue: seller.shiftsCount > 0 
                ? seller.totalRevenue / seller.shiftsCount 
                : 0
        }));
    ...
    async getTodayStats() {
        ...
        return this.getStatsForPeriod(today.toISOString(), tomorrow.toISOString());
    },
    ...
    async getYesterdayStats() {
        ...
        return this.getStatsForPeriod(yesterday.toISOString(), today.toISOString());
    },
    ...
    /**
     * Генерирует предупреждения на основе статистики
     * @private
     */
    generateAlerts(stats) {
        ...
        // Много проданных товаров сегодня
        if (stats.sales.count > 50) {
            alerts.push({
                type: 'info',
                message: 'Высокая активность продаж',
                value: stats.sales.count + ' продаж'
            });
        }
